...
export default function MovieBox() {
  ...
  const giveDateOptions = (lichChieu) => {
    return (
      ...
      lichChieu.map((lich) => {
        ...
        return <option value={convertedDate}>{convertedDate}</option>;
      })
    );
  };
  ...
  const giveTimeOptions = (thoiGian, ngayThang) => {
    if (ngayThang && thoiGian) {
      ...
      for (let i = 0; i < ngayThang.length; i++) {
        ...
        if (ngay === thoiGian) {
          array.push(<option value={gio}>{gio}</option>);
        }
      }
      return array;
    }
  ...
  const handleTimeChange = (event) => {
    setMovie({ ...movie, gioChieu: event.target.value });
  };
  const handleOnSubmit = () => {
    if (movie.gioChieu) {
      setError(null);
    }
    if (movie.gioChieu === "" || movie.gioChieu === "Suất chiếu") {
      setError("Xin hãy chọn lịch chiếu phù hợp");
    }
  };
  useEffect(() => {
    setMovie({ ...movie, rap: "", ngayChieu: "", gioChieu: "" });
  }, [movie.maPhim]);
